...
plot_drainage_network <- function(coords, outfalls, conduits_sf, main, col_arrow){
  ...
  for(i in 1:nrow(conduits_sf)){
    ...
    if(all(list(p0,p1) %in% coords$Name)){
      graphics::arrows(x0 = coords$X[coords$Name == p0], 
                       y0 = coords$Y[coords$Name == p0],
                       x1 = coords$X[coords$Name == p1], 
                       y1 = coords$Y[coords$Name == p1],
             col = col_arrow, length = 0.1)
    }
    if(all(list(p0,p1) %in% coords_O$Name)){
      graphics::arrows(x0 = coords_O$X[coords_O$Name == p0], 
                       y0 = coords_O$Y[coords_O$Name == p0],
                       x1 = coords_O$X[coords_O$Name == p1], 
                       y1 = coords_O$Y[coords_O$Name == p1],
                       col = col_arrow, 
                       length = 0.1)
    }
...
plot_tagged_junctions <- function(junctions, outfalls, conduits_sf, main){
  ...
  dic <- data.frame(tag = c("hill", "sink", "outfall_artificial", "loop", "normal", "start",  "crossing"),
                    pch = c(17,15,19,19,1,19,19), 
                    col = c("red", "red", "red", "lightblue", "black", "black", "blue"),
                    cex = c(1.3,1.3,1.3,1,1,1,1),
                    stringsAsFactors = F)

  graphics::par(mar=c(5.1, 4.1, 4.1, 9), xpd=TRUE)
  plot_drainage_network(coords, 
                        outfalls, 
                        conduits_sf, 
                        main = main, 
                        col_arrow = "black")
  for(tag in unique(coords$tag)){
    graphics::points(coords$X[coords$tag == tag], 
                     coords$Y[coords$tag == tag], 
                     pch = dic$pch[dic$tag == tag],
...
#' @noRd
#' @importFrom sf st_coordinates
plot_path_to_outfall <- function(junctions_u, 
                                 outfalls, 
                                 oid, 
                                 conduits_u, 
                                 main, 
...
#' @noMd
#' @noRd
#' @importFrom graphics plot text rect points legend arrows
plot_cross_section_junctions <- function(junctions, 
                                         conduits, 
                                         connections){
  cross_section_data <- data.frame(Name = connections, 
                                   Length = 0, 
                                   Offset = 0, 
                                   stringsAsFactors = F)
  ...
  graphics::text(cross_section_data$Length, 
                 cross_section_data$Top + 0.3, 
                 cross_section_data$Name, 
                 cex = 0.5, 
                 col = "grey")

  graphics::rect(xleft = cross_section_data$Length - 1, 
                 ybottom = cross_section_data$Bottom,
                 xright = cross_section_data$Length + 1, 
                 ytop = cross_section_data$Top,
                 col = "lightgrey", 
                 border = NA)

  graphics::points(cross_section_data$Length, 
                   cross_section_data$Top, 
                   type = "b", 
                   pch = 19)
  graphics::points(cross_section_data$Length, 
                   cross_section_data$Bottom, 
                   pch = 19, 
                   col = "grey")
  graphics::points(cross_section_data$Length, 
                   cross_section_data$Bottom_pipe, 
                   type = "b", 
                   pch = 19)

  graphics::points(cross_section_data$Length, 
                   cross_section_data$max_bottom, 
                   col = "red", 
                   pch = 4)
  graphics::points(cross_section_data$Length, 
                   cross_section_data$min_bottom, 
                   col = "red",
                   pch = 4)

  graphics::legend("bottomright", 
                   c("top and bottom ", "possible range of bottom", "offset"), 
                   pch = c(19, 4, 19), 
...
plot_slopes <- function(junctions, outfalls, conduits_sf, min_slope, max_slope, main, roundValue = 3){
  ...
  graphics::legend("topright", 
                   inset = c(-0.4,0), 
                   legend = c("slope < min_slope", "slope > max_slope"), 
                   col = c("red", "purple"), 
                   lwd = 1)
...
#' @noRd
#' @importFrom sf st_coordinates
plot_disconnected_conduits <- function(junctions, 
                                       conduits, 
                                       outfalls, 
                                       col_arrow, 
                                       main = "conduits disconnected from outfalls"){
  ...
  # paths to outfall
  network_paths <- track_outfall_to_start_connectivity(outfalls, 
                                                       junctions, 
                                                       conduits)
...
plot_short_cuts <- function(junctions, conduits, outfalls, col_arrow){
  ...
  plot_drainage_network(coords, 
                        outfalls, 
                        conduits, 
                        main = "conduits added to drain sinks", 
                        col_arrow = "black")
...
#' @importFrom graphics par plot points
plot_drainage_network_pipe_diameters <- function(junctions, 
                                                 outfalls, 
                                                 conduits_sf, 
                                                 main, 
                                                 DN_m){
  ...
  dic_scaling <- data.frame(value = DN_m[order(DN_m, decreasing = T)], 
                            stringsAsFactors = F)
  ...
  graphics::legend("topright", 
                   inset = c(-0.3,0), 
                   legend = content_legend$value, 
                   lwd = content_legend$lwd, 
                   col = content_legend$col_arrow)
